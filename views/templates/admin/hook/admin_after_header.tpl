 ...
 *}
<div id="vrpayment_notifications" style="display:none;">
	<li id="vrpayment_manual_notif"class="dropdown">	
		<a href="javascript:void(0);" class="dropdown-toggle vrpayment_manual_notif notifs" data-toggle="dropdown">
			<i class="icon-bullhorn"></i>
				<svg class="vrpayment-icon-bullhorn-svg" viewBox="24 16 64 64">
				...
				{if $manualTotal > 0}					
					<span id="vrpayment_manual_notif_number_wrapper" class="notifs_badge">
						<span id="vrpayment_manual_notif_value">{$manualTotal|escape:'html':'UTF-8'}</span>
					</span>
				{/if}
		</a>
		<div class="dropdown-menu notifs_dropdown" id="dropdown_vrpayment_manual">
			<section id="vrpayment_manual_notif_wrapper" class="notifs_panel" style="width:250px">
				<div class="notifs_panel_header">
					<h3>Manual Tasks</h3>
				</div>
				<div id="list_vrpayment_manual_notif" class="list_notif">
					{if $manualTotal > 0}
						<a href="{$manualUrl|escape:'html':'UTF-8'}" target="_blank">
							<p>{if $manualTotal > 1}
								{l s='There are %s manual tasks that need your attention.' sprintf=$manualTotal mod='vrpayment'}
							{else}
								{l s='There is a manual task that needs your attention.' mod='vrpayment'}
							{/if}
							</p>
						</a>
					{else}
						<span class="no_notifs">
						{l s='There are no manual tasks.' mod='vrpayment'}
						</span>
					{/if}
