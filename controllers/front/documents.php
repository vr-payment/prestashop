...
class VRPaymentDocumentsModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if ($type = Tools::getValue('type')) {
            switch ($type) {
                case 'invoice':
                    if ((bool) Configuration::get(VRPaymentBasemodule::CK_INVOICE)) {
                        $this->processVRPaymentInvoice($order);
                    }
                    break;
                case 'packingSlip':
                    if ((bool) Configuration::get(VRPaymentBasemodule::CK_PACKING_SLIP)) {
                        $this->processVRPaymentPackingSlip($order);
                    }
                    break;
            }
        }
    ...
    private function processVRPaymentInvoice($order)
    {
        try {
    ...
    private function processVRPaymentPackingSlip($order)
    {
        try {
