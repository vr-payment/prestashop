 ...
 */
class VRPaymentBackendDefaultstrategy implements VRPaymentBackendIstrategy
{
    ...
    public function validateAndParseData(Order $order, array $postData)
    {
        ...
        throw new Exception(
            VRPaymentHelper::getModuleInstance()->l('The refund type is not supported.', 'defaultstrategy')
        );
    }
    ...
    public function simplifiedRefund(array $postData)
    {
        ...
        if ($amount >= 0) {
            return array(
                ...
                'vRPaymentOffline' => isset($postData['vrpayment_offline'])
            );
        } else {
            if (! empty($quantity)) {
                throw new Exception(
    ...
    private function validateDataCancelProductType(Order $order, array $postData)
    {
        if (! isset($postData['id_order_detail']) && ! isset($postData['id_customization'])) {
            throw new Exception(
                VRPaymentHelper::getModuleInstance()->l('You must select a product.', 'defaultstrategy')
            );
        } elseif (! isset($postData['cancelQuantity']) && ! isset($postData['cancelCustomizationQuantity'])) {
            throw new Exception(
                VRPaymentHelper::getModuleInstance()->l('You must enter a quantity.', 'defaultstrategy')
            );
        } else {
        ...
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = VRPaymentHelper::roundAmount(
        ...
        if ((float) $reductionAmount > 0) {
            if ($reductionProduct > 0) {
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = VRPaymentHelper::roundAmount($amount * - 1, $currencyCode);
                    if ($totalAmount == 0) {
                        continue;
                    }
                    $discountUniqueIds[] = $uniqueIdBase . '-' . $id;
    ...
    private function applyRefundPartialRefundType(Order $order, array $parsedData)
    {
        ...
        if ($parsedData['generateDiscountRefund']) {
            ...
            $cart_rule->description = sprintf(
                VRPaymentHelper::getModuleInstance()->l('Credit slip for order #%d', 'defaultstrategy'),
                $order->id
            );
    ...
    public function afterApplyRefundActions(Order $order, array $parsedData, array $appliedData)
    {
        ...
        if ($parsedData['refundType'] == self::REFUND_TYPE_PARTIAL_REFUND ||
            ...
            if (Configuration::get(VRPaymentBasemodule::CK_MAIL, null, null, $order->id_shop)) {
                $params = array();
    ...
    public function getVRPaymentRefundType(array $parsedData)
    {
        if ($parsedData['vRPaymentOffline']) {
            return \VRPayment\Sdk\Model\RefundType::MERCHANT_INITIATED_OFFLINE;
        }
        return \VRPayment\Sdk\Model\RefundType::MERCHANT_INITIATED_ONLINE;
    }
    ...
    {
        return VRPaymentVersionadapter::isVoucherOnlyVRPayment($postData);
    }
    ...
    public function processVoucherDeleteRequest(Order $order, array $data)
    {
        ...
        if (Validate::isLoadedObject($order_cart_rule) && $order_cart_rule->id_order == $order->id) {
            ...
            throw new Exception(
                VRPaymentHelper::getModuleInstance()->l('You cannot edit this cart rule.', 'defaultstrategy')
            );
        }
    }
