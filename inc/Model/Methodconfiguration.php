...
class VRPaymentModelMethodconfiguration extends ObjectModel
{
    ...
    public $fee_base = VRPaymentBasemodule::TOTAL_MODE_BOTH_INC;
    ...
    public static $definition = array(
        'table' => 'vrp_method_configuration',
    ...
    {
        $this->title = VRPaymentTools::base64Encode(serialize($title));
    }

    public function getTitle()
    {
        $decoded = VRPaymentTools::base64Decode($this->title, true);
        if ($decoded === false) {
            $decoded = $this->title;
        }
    ...
    {
        $this->description = VRPaymentTools::base64Encode(serialize($description));
    }

    public function getDescription()
    {
        $decoded = VRPaymentTools::base64Decode($this->description, true);
        if ($decoded === false) {
     ...
     * @param int $id
     * @param int $shopId
     * @return VRPaymentModelMethodconfiguration | false
     */
    public static function loadByIdWithChecks($id, $shopId)
    {
        ...
        $collection = new PrestaShopCollection('VRPaymentModelMethodconfiguration');
    ...
    public static function loadByConfigurationAndShop($spaceId, $configurationId, $shopId)
    {
        ...
        if ($result === false) {
            $result = new VRPaymentModelMethodconfiguration();
        }
        return $result;
    }
