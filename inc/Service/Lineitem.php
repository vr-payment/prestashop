 ...
 */
class VRPaymentServiceLineitem extends VRPaymentServiceAbstract
{
    ...
    {
        $currencyCode = VRPaymentHelper::convertCurrencyIdToCode($cart->id_currency);
        $items = array();
        ...
        foreach ($summary['products'] as $productItem) {
            $item = new \VRPayment\Sdk\Model\LineItemCreate();
            $totalAmount = $this->roundAmount((float) $productItem['total_wt'], $currencyCode);
            $item->setAmountIncludingTax($totalAmount);
            $name = empty($productItem['name']) ? VRPaymentHelper::getModuleInstance()->l(
                'Product',
                'lineitem'
            ...
            foreach (array_keys($psTaxes) as $id) {
                ...
                $tax = new \VRPayment\Sdk\Model\TaxCreate();
            ...
            $item->setTaxes($taxes);
            $item->setType(\VRPayment\Sdk\Model\LineItemType::PRODUCT);
            if ($productItem['id_product'] == Configuration::get(VRPaymentBasemodule::CK_FEE_ITEM) ||
                $productItem['id_product'] == Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_ITEM)) {
                $item->setType(\VRPayment\Sdk\Model\LineItemType::FEE);
        ...
        if ($shippingCosts > 0) {
            ...
            $item->setQuantity(1);
            $item->setShippingRequired(false);
            $item->setSku(VRPaymentHelper::getModuleInstance()->l('Shipping', 'lineitem'));
            $name = "";
            ...
            $name = empty($name) ? VRPaymentHelper::getModuleInstance()->l('Shipping', 'lineitem') : $name;
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
            if (! $taxCalculatorFound) {
                $taxRate = 0;
                $taxName = VRPaymentHelper::getModuleInstance()->l('Tax', 'lineitem');
            ...
            $item->setType(\VRPayment\Sdk\Model\LineItemType::SHIPPING);
        ...
        if ($wrappingCosts > 0) {
            ...
            $name = VRPaymentHelper::getModuleInstance()->l('Wrapping Fee', 'lineitem');
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
            $item->setQuantity(1);
            $item->setShippingRequired(false);
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? VRPaymentHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
        ...
        if ($discountOnly && $shippingItem != null) {
            $itemFreeShipping = new \VRPayment\Sdk\Model\LineItemCreate();
            $name = empty($discountOnly['name']) ? VRPaymentHelper::getModuleInstance()->l(
                'Shipping Discount',
                'lineitem'
            ) : $discountOnly['name'];
            $name = $this->trimStringByLength($name, 150);
    ...
    private function extractTaxName($psTax, $cart)
    {
        ...
        if (empty($translated) || strlen($translated) < 2) {
            ...
            return VRPaymentHelper::getModuleInstance()->l('Tax', 'lineitem');
        } else {
            return $translated;
        }
    }
    ...
    public function getItemsFromOrders(array $orders)
    {
        ...
        $cleaned = VRPaymentHelper::cleanupLineItems(
            $items,
            $orderTotal,
            VRPaymentHelper::convertCurrencyIdToCode($order->id_currency)
        );
        return $cleaned;
    }
    ...
    protected function getItemsFromOrdersInner(array $orders, $orderTotal)
    {
        ...
        foreach ($orders as $order) {
            /*@var Order $order */
            $currencyCode = VRPaymentHelper::convertCurrencyIdToCode($order->id_currency);
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? VRPaymentHelper::getModuleInstance()->l(
                    'Product',
                    'lineitem'
                ...
                if ($orderItem['product_id'] == Configuration::get(VRPaymentBasemodule::CK_FEE_ITEM) ||
                    $orderItem['product_id'] == Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_ITEM)) {
                    $item->setType(\VRPayment\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? VRPaymentHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
                ) : $orderCartRule['name'];
                $discountItems = $this->getDiscountItems(
        ...
        if ($cartRule->gift_product != 0) {
            foreach ($existingLineItems as $exisitingLineItem) {
                if ($exisitingLineItem->getUniqueId() ==
                    ...
                    $item->setType(\VRPayment\Sdk\Model\LineItemType::DISCOUNT);
        ...
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                if ($reductionProduct == - 2) {
                    ...
                    $selectedProducts = VRPaymentCartruleaccessor::checkProductRestrictionsStatic(
                        $cartRule,
                        $cartForRule
                    );
     ...
     *
     * @param \VRPayment\Sdk\Model\LineItemCreate $lineItem
     * @return \VRPayment\Sdk\Model\LineItemCreate
     */
    protected function cleanLineItem(\VRPayment\Sdk\Model\LineItemCreate $lineItem)
    {
