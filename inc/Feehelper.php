...
class VRPaymentFeehelper
{
    public static function removeFeeSurchargeProductsFromCart(Cart $cart)
    {
        $surchargeProductId = Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_ITEM);
        ...
        $feeProductId = Configuration::get(VRPaymentBasemodule::CK_FEE_ITEM);
        ...
        VRPaymentVersionadapter::clearCartRuleStaticCache();
    }
    ...
    public static function getSurchargeValues(Cart $cart)
    {
        ...
        $amount = (float) Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_AMOUNT);

        $amountConverted = Tools::convertPrice($amount, Currency::getCurrencyInstance((int) $cart->id_currency));

        $surchargeBaseType = Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_BASE);

        switch ($surchargeBaseType) {
            case VRPaymentBasemodule::TOTAL_MODE_BOTH_INC:
                $taxes = true;
                $surchargeType = Cart::BOTH;
                break;
            case VRPaymentBasemodule::TOTAL_MODE_BOTH_EXC:
                $taxes = false;
                $surchargeType = Cart::BOTH;
                break;
            case VRPaymentBasemodule::TOTAL_MODE_WITHOUT_SHIPPING_INC:
                $taxes = true;
                $surchargeType = Cart::BOTH_WITHOUT_SHIPPING;
                break;
            case VRPaymentBasemodule::TOTAL_MODE_WITHOUT_SHIPPING_EXC:
                $taxes = false;
                $surchargeType = Cart::BOTH_WITHOUT_SHIPPING;
                break;
            case VRPaymentBasemodule::TOTAL_MODE_PRODUCTS_INC:
                $taxes = true;
                $surchargeType = Cart::ONLY_PRODUCTS;
                break;
            case VRPaymentBasemodule::TOTAL_MODE_PRODUCTS_EXC:
                $taxes = false;
                $surchargeType = Cart::ONLY_PRODUCTS;
                break;
        }
        ...
        $surchargeBase = (float) Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_TOTAL);
        ...
        if ($taxGroup != 0) {
            $addressFactory = VRPaymentVersionadapter::getAddressFactory();
            ...
            if ((int) Configuration::get(VRPaymentBasemodule::CK_SURCHARGE_TAX)) {
    ...
    public static function getFeeValues(Cart $cart, VRPaymentModelMethodconfiguration $methodConfiguration)
    {
        $feeProductId = Configuration::get(VRPaymentBasemodule::CK_FEE_ITEM);
        $feeProduct = new Product($feeProductId, false, Configuration::get('PS_LANG_DEFAULT'), $cart->id_shop);
        if (! Validate::isLoadedObject($feeProduct)) {
